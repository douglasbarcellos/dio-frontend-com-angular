...
if (/^\d{6}$/.test(numeroConta)) {
  ...
  if (numArray[0] > 0 && sum <= 30) {
    // Se a condição acima for verdadeira, imprima a mensagem de número de conta válido:
    print('Numero de conta valido.');
  } else {
    // Verifique se o numArray[0] referente ao primeiro dígito é igual a zero:
    if (numArray[0] === 0) {
      // Se o primeiro dígito for igual a zero, imprima a mensagem de número de conta inválido:
      print('Numero de conta invalido: O primeiro digito deve ser maior que zero.');
    } else {
      // Caso contrário, imprima a mensagem de número de conta inválido devido à soma dos dígitos maior que 30:
      print('Numero de conta invalido: A soma dos digitos e maior que 30.');
    }
  }
} else {
  // Se o número de conta não tiver exatamente 6 dígitos, imprima a mensagem de número de conta inválido:
